document.addEventListener('DOMContentLoaded', () => {
    ...
    const DIM_PREREQ_COLOR = { color: 'rgba(41, 128, 185, 0.15)', highlight: 'rgba(52, 152, 219, 0.25)' }; // Dim blue
    const DIM_COREQ_COLOR = { color: 'rgba(39, 174, 96, 0.15)', highlight: 'rgba(46, 204, 113, 0.25)' }; // Dim green
    ...
    const programSuggestionsContainer = document.getElementById('program-suggestions');
    ...
    async function fetchData() {
        try {
            ...
            if (!coursesResponse.ok) throw new Error(`Failed to load ${COURSES_JSON_PATH}: ${coursesResponse.statusText}`);
            if (!programsResponse.ok) throw new Error(`Failed to load ${PROGRAMS_JSON_PATH}: ${programsResponse.statusText}`);
            ...
            console.log('Programs data loaded (parsed programs_and_courses.json):', programsData);

            console.log('Data fetched successfully:', { courses: coursesData.length, programs: programsData ? programsData.length : 'undefined/null' });
        } catch (error) {
    ...
    function populateCategoryFilter(searchTerm = '') {
        ...
        if (!allUniqueCategories) { 
            ...
            console.error("allUniqueCategories is not populated when populateCategoryFilter is called.");
            if (categorySuggestionsContainer) categorySuggestionsContainer.style.display = 'none';
            return;
        }
        
        if (allUniqueCategories.length === 0 && searchTerm === '') { // Only hide if truly empty and no search term
        ...
        if (categoryFilterSelect && categoryFilterSelect.options.length <= 1) { 
            ...
            allUniqueCategories.forEach(cat => {
                if (cat) { // Ensure category is not null or empty string
        ...
        const filteredCategories = allUniqueCategories.filter(category => 
            category && category.toLowerCase().includes(searchTermLower) // Add null check for category
        );

        let suggestionsMade = false;

        // Add "All Categories" to suggestions if it matches search or if search is empty
        if ("--- all categories ---".includes(searchTermLower) || searchTermLower === "") {
            const div = document.createElement('div');
        ...
        filteredCategories.forEach(category => {
            if (category) { // Ensure category is not null or empty string
                const div = document.createElement('div');
    ...
    function populateProgramSelect() {
        console.log('Entering populateProgramSelect. programsData:', programsData, 'Length:', programsData ? programsData.length : 'N/A');
        ...
        programsData.forEach((program, index) => {
            console.log(`Processing program at index ${index}:`, JSON.stringify(program, null, 2)); // Log the program object for inspection
            if (program && typeof program.program === 'string' && program.program.trim() !== '') {
                const option = document.createElement('option');
                option.value = program.program; // Use program.program
                option.textContent = program.program; // Use program.program
                programSelect.appendChild(option);
            } else {
                console.warn(`Skipping program at index ${index} due to missing or empty 'program' property. Program data:`, program);
            }
        });
    }
    ...
    function getRelevantCoursesForProgram(programCourseCodes) {
        ...
        const queue = programCourseCodes.filter(code => coursesData.find(c => c.code === code));
        ...
        while (head < queue.length) {
            ...
            const addDependencies = (requirements) => {
                ...
                requirements.forEach(req => {
                    if (req.type === 'COURSE') {
                        if (coursesData.find(c => c.code === req.code) && !relevantCodes.has(req.code)) {
        ...
        displaySet.forEach(courseCode => {
            ...
            if (course.prerequisites_parsed) {
                course.prerequisites_parsed.forEach(reqDetail => {
                    addRequirementGraphElements(reqDetail, course.code, 'prereq', displaySet, nodesArray, edgesArray, idCounter, course.code);
                });
            }
            ...
            if (course.corequisites_parsed) {
                course.corequisites_parsed.forEach(reqDetail => {
                    addRequirementGraphElements(reqDetail, course.code, 'coreq', displaySet, nodesArray, edgesArray, idCounter, course.code);
                });
            }
        });
    ...
    function addRequirementGraphElements(requirement, targetNodeId, type, displaySet, nodesArray, edgesArray, idCounter, originalCourseCode) {
        ...
        if (requirement.type === 'COURSE') {
            ...
            if (courseData) { // Check if the course exists in the main dataset
                ...
                const edge = {
                    ...
                    dashes: type === 'coreq',
                    title: `${courseCode} ${type === 'prereq' ? 'is a prerequisite for' : 'is a corequisite with'} ${targetNodeId.startsWith('logic_') ? 'a condition group for' : ''} ${originalCourseCode}`,
                    baseColorType: type,
                ...
                console.warn(`Course ${courseCode} referenced as a requirement for ${originalCourseCode} but not found in coursesData.`);
            }
            ...
            const edgeToTarget = {
                ...
                title: `This ${operator} group is a ${type} for ${targetNodeId.startsWith('logic_') ? 'another condition group for' : ''} ${originalCourseCode}`,
            ...
            if (requirement.conditions && requirement.conditions.length > 0) {
                requirement.conditions.forEach(condition => {
                    addRequirementGraphElements(condition, logicNodeId, type, displaySet, nodesArray, edgesArray, idCounter, originalCourseCode);
                });
            }
            ...
            const edgeToTarget = {
                ...
                title: `This textual condition is a ${type} for ${targetNodeId.startsWith('logic_') ? 'a condition group for' : ''} ${originalCourseCode}`,
                baseColorType: type,
            ...
            if (requirement.count === 1) {
                ...
                const edgeToTarget = {
                    ...
                    color: type === 'prereq' ? DIM_PREREQ_COLOR : DIM_COREQ_COLOR,
                    dashes: false,
                    title: `This OR group (1 of list) is a ${type} for ${targetNodeId.startsWith('logic_') ? 'another condition for' : ''} ${originalCourseCode}`,
                    baseColorType: type,
                ...
                if (requirement.conditions && requirement.conditions.length > 0) {
                    requirement.conditions.forEach(itemInList => {
                        addRequirementGraphElements(itemInList, orNodeId, type, displaySet, nodesArray, edgesArray, idCounter, originalCourseCode);
                    });
                }
            } else {
                ...
                const edgeToMainTarget = {
                    ...
                    color: type === 'prereq' ? DIM_PREREQ_COLOR : DIM_COREQ_COLOR,
                    dashes: false,
                    title: `This '${nodeLabel}' group is a ${type} for ${targetNodeId.startsWith('logic_') ? 'another condition group for' : ''} ${originalCourseCode}`,
                    baseColorType: type,
                ...
                if (requirement.conditions && requirement.conditions.length > 0) {
                    requirement.conditions.forEach(itemInList => {
                        addRequirementGraphElements(itemInList, nOfListNodeId, type, displaySet, nodesArray, edgesArray, idCounter, originalCourseCode);
                    });
                }
            }
        }
    ...
    function displayGraphForCourses(courseCodesToDisplay) {
        ...
        if (!courseCodesToDisplay || courseCodesToDisplay.length === 0) {
            ...
            if (network) network.setData({ nodes: allNodesDataSet, edges: allEdgesDataSet });
        ...
        nodes.forEach(node => {
            if (!uniqueNodesById.has(node.id)) {
                ...
                console.warn(`Duplicate node ID encountered and removed before rendering: ${node.id}.`);
            }
        });
        ...
        edges.forEach(edge => {
            if (!uniqueEdgesById.has(edge.id)) {
                ...
                console.warn(`Duplicate edge ID encountered and removed before rendering: ${edge.id}.`);
            }
        });
    ...
    function initializeNetworkShell() {
        ...
        groups: {
            ...
            AndNode: { shape: 'diamond', size: 10, color: { background: '#bdc3c7', border: '#95a5a6' }, font: { size: 10, color: '#34495e' }, label: 'AND' },
            OrNode: { shape: 'ellipse', size: 10, color: { background: '#bdc3c7', border: '#95a5a6' }, font: { size: 10, color: '#34495e' }, label: 'OR' },
            TextualNode: { shape: 'box', color: { background: '#e0e0e0', border: '#cccccc' }, font: { multi: true, size: 10, color: '#555555', align: 'left' }, margin: 8, widthConstraint: { maximum: 150 } },
            NOfListNode: { shape: 'hexagon', size: 12, color: { background: '#a2ded0', border: '#87bcaf' }, font: { size: 10, color: '#34495e' } }
        }
    };
    ...
    function setupEventListeners() {
        ...
        if (programSuggestionsContainer) {
            programSuggestionsContainer.addEventListener('click', handleSuggestionClick);
        }
        ...
        if (categorySearchInput) {
            ...
            categorySearchInput.addEventListener('focus', () => populateCategoryFilter(categorySearchInput.value)); 
        }

        if (categorySuggestionsContainer) {
            categorySuggestionsContainer.addEventListener('click', handleCategorySuggestionClick);
        }
        ...
        document.addEventListener('click', function(event) {
            ...
            if (programSearchInput && programSuggestionsContainer) {
                if (!programSearchInput.contains(event.target) && !programSuggestionsContainer.contains(event.target)) {
                    programSuggestionsContainer.style.display = 'none';
                }
            }
            // Hide category suggestions
            if (categorySearchInput && categorySuggestionsContainer) {
                if (!categorySearchInput.contains(event.target) && !categorySuggestionsContainer.contains(event.target)) {
                    categorySuggestionsContainer.style.display = 'none';
                }
            }
        });
    ...
    function handleNetworkClick(params) {
        ...
        const edgesToUpdate = allEdgesDataSet.get({ returnType: 'Array' }).map(edge => {
            let newColorObj;
            ...
            if (clickedNodeId && (edge.from === clickedNodeId || edge.to === clickedNodeId)) {
                newColorObj = baseType === 'prereq' ? HIGHLIGHT_PREREQ_COLOR : HIGHLIGHT_COREQ_COLOR;
            } else {
                newColorObj = baseType === 'prereq' ? DIM_PREREQ_COLOR : DIM_COREQ_COLOR;
    ...
    if (clickedNodeId) {
        ...
        if (network && typeof network.getConnectedNodes === 'function') { 
             network.getConnectedNodes(clickedNodeId).forEach(nodeId => connectedNodes.add(nodeId));
        }
...
function updateSidebar(course) {
    let prereqsHTML = course.prerequisites_raw ? `<div class="raw-reqs">${course.prerequisites_raw}</div>` : 'None';
    let coreqsHTML = course.corequisites_raw ? `<div class="raw-reqs">${course.corequisites_raw}</div>` : 'None';
    ...
    function handleProgramChange() {
        ...
        if (selectedProgramName === "") { // Handle the "--- Select a Program ---" option
            displayGraphForCourses([]); // Clear the graph
            hideLoadingIndicator();
        } else if (selectedProgramName === 'all') { // Handle "Show All Programs"
            const allCourseCodes = coursesData.map(c => c.code);
            displayGraphForCourses(allCourseCodes);
            hideLoadingIndicator();
        } else { // Handle a specific program selection
            const program = programsData.find(p => p.program === selectedProgramName);
            if (program && program.courses) {
                ...
                console.warn(`Program "${selectedProgramName}" not found or has no courses defined.`);
    ...
    function handleSearch() {
        ...
        if (allNodesDataSet.get(searchTerm)) { // Search in current DataSet
            ...
            alert(`Course code "${searchTerm}" not found in the current view. Try selecting "Show All Programs" or a different program.`);
        }
    }
